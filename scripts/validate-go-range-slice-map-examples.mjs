...
const expected = new Map([
  ["go-slices-maps-range-over-slice-map-interview-basics", "comparison_table"],
  ["go-slices-maps-range-over-slice-map-when-to-use", "flow_diagram"],
  ["go-slices-maps-range-over-slice-map-common-mistake", "comparison_table"],
  ["go-slices-maps-range-over-slice-map-compare", "flow_diagram"],
]);
...
if (document.questions.length !== expected.size) {
  throw new Error("Expected four range questions, found " + document.questions.length);
}
...
for (const question of document.questions) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ie-go-range-collection-"));
  try {
    fs.writeFileSync(
      path.join(tempRoot, "go.mod"),
      "module example.com/rangecollectioncheck\n\ngo 1.22\n",
    );
    ...
    const safePrefix = path.join(os.tmpdir(), "ie-go-range-collection-");
...
if (quickHashes.size !== document.questions.length) {
  throw new Error("Quick Revision content is duplicated between range questions");
}
if (exampleHashes.size !== document.questions.length) {
  throw new Error("Complete Go examples are duplicated between range questions");
}

console.log("Validated all range-over-collection learning zones and Go examples.");

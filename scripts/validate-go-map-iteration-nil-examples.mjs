...
const expected = new Map([
  ["go-slices-maps-map-iteration-and-nil-interview-basics", "comparison_table"],
  ["go-slices-maps-map-iteration-and-nil-when-to-use", "flow_diagram"],
  ["go-slices-maps-map-iteration-and-nil-common-mistake", "flow_diagram"],
  ["go-slices-maps-map-iteration-and-nil-compare", "comparison_table"],
]);
...
if (document.questions.length !== expected.size) {
  throw new Error("Expected four map iteration questions, found " + document.questions.length);
}
...
for (const question of document.questions) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ie-go-map-iteration-"));
  try {
    fs.writeFileSync(
      path.join(tempRoot, "go.mod"),
      "module example.com/mapiterationcheck\n\ngo 1.22\n",
    );
    ...
    const safePrefix = path.join(os.tmpdir(), "ie-go-map-iteration-");
...
if (quickHashes.size !== document.questions.length) {
  throw new Error("Quick Revision content is duplicated between map questions");
}
if (exampleHashes.size !== document.questions.length) {
  throw new Error("Complete Go examples are duplicated between map questions");
}

console.log("Validated all map-iteration learning zones and Go examples.");

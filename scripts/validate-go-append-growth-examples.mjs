...
const expected = new Map([
  ["go-slices-maps-append-and-growth-interview-basics", "flow_diagram"],
  ["go-slices-maps-append-and-growth-when-to-use", "flow_diagram"],
  ["go-slices-maps-append-and-growth-common-mistake", "comparison_table"],
  ["go-slices-maps-append-and-growth-compare", "comparison_table"],
]);
...
if (document.questions.length !== expected.size) {
  throw new Error(
    "Expected four append-and-growth questions, found " + document.questions.length,
  );
}

for (const question of document.questions) {
  const expectedVisual = expected.get(question.slug);
  if (!expectedVisual) {
  ...
  const visual = sections.find((section) => section.type === expectedVisual);
  ...
  if (supportCount + sectionVisualCount !== 1) {
    throw new Error(
      "Expected exactly one purposeful whole-answer visual for " + question.slug,
    );
  }
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ie-go-append-growth-"));
  try {
    fs.writeFileSync(
      path.join(tempRoot, "go.mod"),
      "module example.com/appendgrowthcheck\n\ngo 1.22\n",
    );
    ...
    const safePrefix = path.join(os.tmpdir(), "ie-go-append-growth-");
...
if (quickHashes.size !== document.questions.length) {
  throw new Error("Quick Revision content is duplicated between append questions");
}
if (exampleHashes.size !== document.questions.length) {
  throw new Error("Complete Go examples are duplicated between append questions");
}

console.log("Validated all append-and-growth learning zones and Go examples.");

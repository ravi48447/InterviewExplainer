...
const expected = new Map([
  ...
  [
    ...
    {
      visual: "comparison_table",
      output: ["point: true origin", "record contents: true"].join("\n"),
    },
  ],
]);
...
if (document.questions.length !== expected.size) {
  throw new Error("Expected four comparison questions, found " + document.questions.length);
}
...
for (const question of document.questions) {
  ...
  if (!/^package\s+main$/m.test(code) || !/\bfunc\s+main\s*\(\s*\)/.test(code)) {
    throw new Error("Go example is not a runnable program for " + question.slug);
  }
  ...
  try {
    fs.writeFileSync(
      ...
      "module example.com/comparisonscheck\n\ngo 1.22\n",
    );
    ...
    const safePrefix = path.join(os.tmpdir(), "ie-go-comparisons-");
...
if (codeHashes.size !== document.questions.length) {
  throw new Error("Complete Go examples are duplicated between questions");
}

console.log("Validated all Go collection-comparison lessons and examples.");

...
const expected = new Map([
  ["go-slices-maps-maps-make-and-literal-interview-basics", "comparison_table"],
  ["go-slices-maps-maps-make-and-literal-when-to-use", "comparison_table"],
  ["go-slices-maps-maps-make-and-literal-common-mistake", "comparison_table"],
  ["go-slices-maps-maps-make-and-literal-compare", "flow_diagram"],
]);
...
if (document.questions.length !== expected.size) {
  throw new Error("Expected four map questions, found " + document.questions.length);
}
...
for (const question of document.questions) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ie-go-maps-make-"));
  try {
    fs.writeFileSync(
      path.join(tempRoot, "go.mod"),
      "module example.com/mapsmakecheck\n\ngo 1.22\n",
    );
    ...
    const safePrefix = path.join(os.tmpdir(), "ie-go-maps-make-");
...
console.log("Validated all map-construction learning zones and Go examples.");

    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50/20 to-yellow-50/20">
      <div className="w-full min-w-0 px-4 sm:px-6 lg:px-8 py-12">
        <nav className="flex items-center gap-2 text-xs text-slate-500 mb-8">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="text-slate-700 font-semibold">Business Analyst</span>
        </nav>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          {TOPICS.map(topic => (
            <div key={topic.name} className="rounded-xl border border-amber-200 bg-white p-5 opacity-70">
              <div className="text-2xl mb-3">{topic.emoji}</div>
              <h3 className="font-black text-slate-900 mb-2">{topic.name}</h3>

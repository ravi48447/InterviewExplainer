...
const LEVEL_META: Record<Level, { label: string; color: string; colorClass: string; range: string }> = {
  beginner: { label: "Beginner", color: "#10b981", colorClass: "bg-emerald-100 text-emerald-700 border-emerald-200", range: "0–2 years" },
  intermediate: { label: "Intermediate", color: "#f59e0b", colorClass: "bg-amber-100 text-amber-700 border-amber-200", range: "2–5 years" },
  advanced: { label: "Advanced", color: "#ef4444", colorClass: "bg-red-100 text-red-700 border-red-200", range: "5+ years" },
};
          ...
          <aside className="hidden lg:flex w-[280px] shrink-0 flex-col gap-4 self-start sticky top-6">
            <div className="rounded-xl border border-slate-200 bg-white/90 backdrop-blur-sm shadow-sm overflow-hidden">
              <div className="px-4 py-3 bg-gradient-to-r from-slate-100 to-slate-50 border-b border-slate-200">
                <Link href="/interview" className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-slate-600 hover:text-[#2e64e5] transition-colors">
              ...
              <div className="rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 shadow-sm overflow-hidden">
                <div className="px-4 py-3 bg-gradient-to-r from-blue-100 to-blue-50 border-b border-blue-200">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-bold text-slate-800 uppercase tracking-wide">Experience Level</h3>
                  </div>
                </div>
                    ...
                    return (
                      <Link
                        ...
                        className={`flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold rounded-lg transition-all ${
                          isActive
                            ? "bg-white text-slate-900 border border-blue-200 shadow-sm"
                            : "text-slate-600 hover:text-slate-800 hover:bg-white/50 border border-transparent"
                        }`}
                      >
                        <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: lvlMeta.color }} />
                        <span>{lvlMeta.label}</span>
                        <span className="text-slate-400 ml-auto">{lvlMeta.range}</span>
                      </Link>
                    );
                  })}
                </div>
            ...
            <div className="rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 shadow-sm p-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs">
                  <span className="text-slate-600">Stacks Completed</span>
                  <span className="font-bold text-slate-800">0/{stacks.length}</span>
                </div>
          ...
          <main className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-bold text-slate-900">{stacks.length}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-bold text-slate-900">{meta.label} <span className="text-slate-500 font-normal">({meta.range})</span></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 pb-10">
              {stackData.map((stack, sIdx) => (
                <div key={stack.slug} className="border rounded-[12px] border-slate-200 shadow-sm bg-[#f8f9fa] overflow-hidden hover:border-slate-300 hover:shadow-md transition-all">
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center">
                    <div className="flex-1 flex gap-3 items-start sm:items-center p-4 sm:px-5">
                      <div className="shrink-0 w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400 flex items-center justify-center">
                        <Layers className="h-3.5 w-3.5" />
                      </div>
                      <div>
                        <h3 className="text-[15px] font-bold text-slate-800 tracking-tight leading-tight mb-0.5">{stack.name}</h3>
                        {stack.description && <p className="text-[13px] text-slate-500 leading-snug line-clamp-1">{stack.description}</p>}
                      </div>
                    </div>
                    <div className="flex items-center gap-3 shrink-0 px-4 pb-3 sm:pb-0 sm:pr-5">
                      ...
                      <Link
                        ...
                        className="flex items-center justify-center h-8 px-3 rounded-md bg-white border border-slate-200 text-[#2e64e5] text-[11px] font-bold uppercase tracking-wider hover:border-[#2e64e5] hover:bg-[#2e64e5]/5 transition-colors shadow-sm"
                      >
                    ...
                    <div className="border-t border-slate-100 bg-white px-3 sm:px-4 py-3">
                      ...
                      {stack.questionCount > 5 && (
                        <div className="mt-3 text-center border-t border-slate-100 pt-3">
                          <Link href={`${basePath}/${stack.slug}`} className="text-[#2e64e5] hover:text-blue-700 text-[12px] font-bold tracking-wide flex items-center justify-center gap-1">
          ...
          <aside className="hidden xl:flex w-[300px] shrink-0 flex-col gap-4 self-start sticky top-6">
              ...
              <div className="p-4">
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-white rounded-lg p-3 border border-purple-200 shadow-sm">
                    ...
                    <div className="text-2xl font-black text-slate-900 leading-none">{stacks.length}</div>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-purple-200 shadow-sm">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-purple-600 mb-1">Questions</div>
                    <div className="text-2xl font-black text-slate-900 leading-none">{totalQuestions}</div>
                  </div>
                </div>
                <div className="space-y-2 pt-3 border-t border-purple-200">
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-slate-900">{toDisplayName(lang)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-600 font-medium">Track</span>
                    <span className="font-bold text-slate-900">{toDisplayName(track)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white/90 backdrop-blur-sm shadow-sm overflow-hidden">
              ...
              <div className="p-3 space-y-2">
                <Link href="/interview" className="flex items-center justify-between px-3 py-2 text-xs font-semibold text-slate-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all border border-transparent hover:border-blue-200">

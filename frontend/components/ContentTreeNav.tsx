...
export default function ContentTreeNav({
  ...
  const treeContent = (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Header */}
      <div className="shrink-0 border-b border-slate-800/80 bg-[#0a0e1a]">
        <div className="flex items-center justify-between gap-2 px-4 py-3">
          <Link
            href={`/${domainSlug}`}
            className="flex items-center gap-1.5 text-sm font-bold text-slate-400 hover:text-blue-400 transition-colors truncate"
          >
          ...
          <button
            ...
            className="shrink-0 p-1.5 rounded-lg hover:bg-slate-800 transition-colors text-slate-500 hover:text-slate-200"
          >
            {collapsed
        ...
        {!collapsed && (
          <div className="px-4 pb-3">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px]">
              <span className="rounded-md border border-slate-700 bg-slate-800 px-2 py-1 font-bold text-slate-300">
                {totalStacks} modules
              </span>
              <span className="rounded-md border border-blue-700/50 bg-blue-900/40 px-2 py-1 font-bold text-blue-300">
              ...
              <div key={cat.slug} className="mb-2">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    hasCatActive
                      ? "bg-gradient-to-r from-amber-900/70 to-amber-800/40 text-amber-200 ring-1 ring-amber-500/50 shadow-sm"
                      : "bg-slate-800 text-slate-200 hover:bg-slate-700/80 hover:text-white border border-slate-700/50"
                  )}
                >
                  {/* Pillar number badge */}
                  <span className={cn(
                    "shrink-0 w-7 h-7 rounded-lg flex items-center justify-center text-[12px] font-black",
                    hasCatActive ? "bg-amber-500/40 text-amber-200" : "bg-slate-600/80 text-slate-300"
                  )}>
                    {catIdx + 1}
                  </span>
                  ...
                  <span className={cn(
                    ...
                    hasCatActive ? "bg-amber-800/60 text-amber-300" : "bg-slate-700 text-slate-400"
                  )}>
                    {cat.stacks.length}
                  </span>
                  <ChevronRight className={cn(
                    ...
                    isCatOpen && "rotate-90",
                    hasCatActive ? "text-amber-400" : "text-slate-500"
                  )} />
                </button>

                {/* ── Stacks ── */}
                {isCatOpen && (
                  <div className="ml-1 mt-1.5 space-y-0.5 border-l-2 border-slate-600/60 pl-3">
                    {cat.stacks.map(stack => {
                      ...
                      return (
                        <div key={stack.slug}>
                          ...
                          <div className={cn(
                            ...
                            >
                              ...
                              <div className={cn(
                                ...
                                isActiveStack ? "bg-blue-400" : "bg-transparent"
                              )} />
                              <span className={cn(
                                "flex-1 text-[13px] leading-tight truncate",
                                isActiveStack ? "font-bold text-blue-100" : "font-semibold text-slate-300 hover:text-slate-100"
                              )}>
                                {stack.name}
                              </span>
                              <span className={cn(
                                "text-[11px] shrink-0 mr-1 px-1.5 py-0.5 rounded font-bold",
                                isActiveStack ? "bg-blue-700/60 text-blue-200" : "text-slate-500"
                              )}>
                                {stack.questionCount}
                              </span>
                              {isLoadingStack ? (
                                ...
                                <ChevronRight className={cn(
                                  ...
                                  isActiveStack ? "text-blue-400" : "text-slate-500"
                                )} />
                              )}
                            </button>
                          </div>
                          ...
                          {isExpStack && subs.length > 0 && (() => {
                            ...
                            if (isFlatStack) {
                              return (
                                <div className="ml-3 border-l border-slate-600/50 pl-2 mt-1 mb-1 space-y-0.5">
                                  {flatQuestions.map(q => {
                                    ...
                                    return (
                                      <Link
                                        ...
                                        className={cn(
                                          ...
                                          isActiveQ
                                            ? "bg-emerald-600 text-white font-bold shadow-sm ring-1 ring-emerald-500"
                                            : "text-slate-400 hover:bg-slate-700/60 hover:text-slate-200"
                                        )}
                                      >
                                        <span className={cn("mt-0.5 shrink-0", isActiveQ ? "text-emerald-200" : "text-slate-600")}>
                                    ...
                                    <div key={sc.slug}>
                                      <button
                                        ...
                                        className={cn(
                                          ...
                                          hasActiveQ
                                            ? "bg-amber-900/40 text-amber-300 font-bold ring-1 ring-amber-700/40"
                                            : "text-slate-300 hover:bg-slate-700/60 font-semibold hover:text-slate-100"
                                        )}
                                      >
                                        {isExpSubcat
                                        ...
                                        <span className="text-[11px] font-bold px-1.5 py-0.5 rounded bg-amber-900/40 text-amber-400 shrink-0">
                                          {sc.questionCount}
                                        </span>
                                        <ChevronRight
                                          className={cn(
                                            "h-3 w-3 text-slate-500 transition-transform duration-200 shrink-0",
                                      ...
                                      {isExpSubcat && (
                                        <div className="ml-3 border-l border-slate-600/50 pl-1.5 space-y-0.5 mt-0.5 mb-1.5">
                                          {sc.questions.map(q => {
                                            const isActiveQ = q.slug === activeQuestionSlug;
                                            const qHref = `/${domainSlug}/${stack.slug}/${q.slug}`;
                                            return (
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          "bg-slate-900 border border-slate-700/60 rounded-2xl shadow-xl",
          "overflow-hidden",
      ...
      >
        ...
        {!collapsed && (
          ...
          >
            ...
            <div className="absolute inset-y-0 right-0 w-px bg-blue-500/0 group-hover/handle:bg-blue-500/50 transition-colors duration-150" />
            ...
            <div className="absolute top-1/2 -translate-y-1/2 right-0 translate-x-1/2
              ...
              <span className="text-[9px] font-black text-slate-300 leading-none select-none">‹</span>
              <span className="text-[9px] font-black text-slate-300 leading-none select-none">›</span>
            </div>
          </div>
        )}
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          />
          {/* Drawer panel */}
          <aside className="relative z-10 w-[280px] bg-white shadow-2xl flex flex-col h-full overflow-hidden">

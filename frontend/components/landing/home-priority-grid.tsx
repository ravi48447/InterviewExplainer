...
> = {
  network: {
    ...
    node: <Network className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  zap: {
    wrap: "bg-gradient-to-br from-amber-500 to-orange-600",
    node: <Zap className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  layers: {
    wrap: "bg-gradient-to-br from-indigo-500 to-blue-600",
    node: <Layers className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  link2: {
    wrap: "bg-gradient-to-br from-violet-500 to-purple-600",
    node: <Link2 className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  radio: {
    wrap: "bg-gradient-to-br from-rose-500 to-pink-600",
    node: <Radio className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  cloud: {
    wrap: "bg-gradient-to-br from-sky-500 to-cyan-600",
    node: <Cloud className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  layout: {
    wrap: "bg-gradient-to-br from-slate-600 to-slate-800",
    node: <LayoutGrid className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
  braces: {
    wrap: "bg-gradient-to-br from-orange-500 to-amber-600",
    node: <Braces className="h-5 w-5 text-primary-foreground dark:text-foreground" aria-hidden />,
  },
};
      ...
      <div className="w-full px-6 sm:px-12 lg:px-20">
        <div className="w-full min-w-0">
          <div className="text-center mb-8 sm:mb-10">
            ...
            <p className="text-sm sm:text-base text-secondary max-w-3xl mx-auto leading-relaxed">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
            {picks.map((p) => (
              <li key={`${p.headline}::${p.href}`}>
                <Link
                  href={p.href}
                  className="group flex h-full flex-col rounded-xl border border-border bg-background p-4 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all"
                >
                  <div className="flex items-start gap-3 mb-3">
                    <PickIcon icon={p.icon} />
                    <div className="flex-1 min-w-0 text-left">
                      <h3 className="text-[15px] font-bold text-foreground leading-snug group-hover:text-indigo-700 transition-colors">
                        {p.headline}
                      </h3>
                      <p className="mt-1 text-xs text-secondary leading-snug line-clamp-3">
                        {p.tagline}
                      </p>
                    </div>
                  </div>

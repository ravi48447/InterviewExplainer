...
export function HeroDashboardVisual() {
    return (
        <div className="relative w-full h-full min-h-[600px] flex items-center justify-center perspective-[2000px]">
            ...
            >
                {/* Mac Screen Bezel */}
                <div className="relative w-[750px] h-[470px] bg-slate-950 rounded-[18px] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden border-[10px] border-slate-950">
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-5 overflow-hidden relative">
                        ...
                        <div className="h-[calc(100%-56px)] grid grid-cols-12 gap-3">
                            ...
                            <div className="col-span-3 space-y-3">
                                ...
                                {/* Quick Stats */}
                                {[
                                    { label: 'Questions', value: '247', icon: Target, iconClass: 'text-indigo-400' },
                                    { label: 'Study Time', value: '42h', icon: Clock, iconClass: 'text-blue-400' },
                                    { label: 'Accuracy', value: '85%', icon: Award, iconClass: 'text-emerald-400' },
                                    { label: 'Streak', value: '12d', icon: Zap, iconClass: 'text-orange-400' }
                                ].map((stat, i) => (
                                    <motion.div
                                        key={i}
                                ...
                                <div className="grid grid-cols-2 gap-3 h-[45%]">
                                    ...
                                    <div className="dark:bg-surface/50 border border-border/50 rounded-lg p-3">
                                        ...
                                        <div className="space-y-2.5">
                                            {[
                                                { level: 'Easy', solved: 120, total: 135, percent: 89, dotClass: 'bg-emerald-400', barClass: 'bg-emerald-500' },
                                                { level: 'Medium', solved: 95, total: 140, percent: 68, dotClass: 'bg-amber-400', barClass: 'bg-amber-500' },
                                                { level: 'Hard', solved: 32, total: 85, percent: 38, dotClass: 'bg-red-400', barClass: 'bg-red-500' }
                                            ].map((item, i) => (
                                                <div key={item.level}>
                                                    <div className="flex items-center justify-between mb-1">
                                                        <div className="flex items-center gap-2">
                                                            <div className={`w-2 h-2 rounded-full ${item.dotClass}`} />
                                                            <span className="text-[11px] font-semibold text-slate-300">{item.level}</span>
                                                        </div>
                                                        <div className="text-[11px] text-slate-400">
                                                            <span className="font-bold text-slate-300">{item.solved}</span>/{item.total}
                                                        </div>
                                                    </div>
                                        ...
                                        <div className="space-y-1.5">
                                            {[
                                                ...
                                                <motion.div
                                                    ...
                                                    className="flex items-start gap-2 p-1.5 rounded hover:bg-slate-700/30 transition-colors"
                                                >
                ...
                <div className="h-1 bg-slate-950 rounded-b-3xl mx-12" style={{ transform: "rotateX(-5deg) translateZ(-12px)" }}></div>
            </motion.div>
        </div>
    );
}
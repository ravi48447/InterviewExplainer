...
const FREQ_META: Record<string, { label: string; className: string }> = {
  "very-high": { label: "Hot", className: "text-rose-600 bg-rose-50 border-rose-200" },
  high: { label: "Freq", className: "text-amber-700 bg-amber-50 border-amber-200" },
};
...
export function DSAProblemRow({
  ...
  const inner = (
    <div className="flex items-center gap-3 w-full min-w-0">
      {/* Number */}
      <span className="text-[11px] font-mono tabular-nums text-slate-400 w-7 shrink-0 text-right">
      ...
      {/* Status */}
      {authored ? (
        <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-1.5 py-0.5 shrink-0">
          Explained
        </span>
      ) : (
      ...
      {/* Arrow */}
      {authored && (
        <ArrowRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-violet-500 shrink-0 transition-colors" />
      )}
    </div>
  );

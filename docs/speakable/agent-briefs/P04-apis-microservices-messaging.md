# Pillar P04 — APIs, Microservices & Messaging agent brief

> Phase 3a/3b agent brief generated by `scripts/build_agent_briefs.py`. Self-contained: an agent reading only this brief, the lint script `scripts/audit_speakable.py`, the codex JSONs, and the 7 golden references embedded below can refine this pillar's queue to lint-green output. Phase 3a smoke gate: draft **only the first question** in `content/_audits/agent-queues/P04-queue.csv`, halt for human review. (Per the SPEAKABLE-PHASE-3A-PROMPT.md §1.5 + §7.4.)

---

## Identity & scope

- **Pillar:** P04 / APIs, Microservices & Messaging
- **Pillar register:** see slice below.
- **Number of questions in scope** (excludes 7 goldens + 30 priority_handcraft + already-approved): **113**
- **Question list:** see `content/_audits/agent-queues/P04-queue.csv` (columns: slug, archetype, confidence, importance, difficulty, module, topic, file).
...
|---:|---|---|---|---|---|
| 1 | `message-broker-vs-message-bus-vs-event-bus-semantics` | B | high | high | `messaging-events/comparisons` |
| 2 | `kafka-log-compaction-and-retention` | B | medium | high | `messaging-events/kafka-architecture` |
| 3 | `kafka-streams-kstream-ktable-fundamentals` | B | medium | high | `messaging-events/stream-processing` |
| 4 | `webhook-reliability-retry-exponential-backoff` | A | medium | high | `messaging-events/webhooks` |
| 5 | `webhook-security-hmac-signature-verification` | A | medium | high | `messaging-events/webhooks` |
| 6 | `microservices-vs-serverless-vs-monolith-decision` | B | high | high | `microservices/comparisons` |
| 7 | `orchestration-vs-choreography-saga-comparison` | B | high | high | `microservices/comparisons` |
| 8 | `cqrs-with-event-sourcing-spring-boot` | A | medium | high | `messaging-events/cqrs` |
| 9 | `kafka-broker-partition-leadership-isr` | A | medium | high | `messaging-events/kafka-architecture` |
| 10 | `kafka-controller-election-kraft-vs-zookeeper` | B | medium | high | `messaging-events/kafka-architecture` |
| 11 | `kafka-replication-and-durability` | A | medium | high | `messaging-events/kafka-architecture` |
| 12 | `designing-idempotent-payment-microservice` | D | high | high | `microservices/scenario-based` |
| 13 | `migrating-monolith-to-microservices-sequencing` | D | high | high | `microservices/scenario-based` |
| 14 | `embedded-kafka-testing-spring-boot` | A | medium | high | `messaging-events/kafka-architecture` |
| 15 | `acknowledgment-mode-manual-vs-auto` | B | medium | high | `messaging-events/kafka-patterns` |
| 16 | `kafka-consumer-group-rebalancing` | A | medium | high | `messaging-events/kafka-patterns` |
| 17 | `kafka-consumer-lag-monitoring` | A | medium | high | `messaging-events/kafka-patterns` |
| 18 | `kafka-headers-message-metadata` | A | medium | high | `messaging-events/kafka-patterns` |
| 19 | `kafka-json-avro-serialization-spring` | B | medium | high | `messaging-events/kafka-patterns` |
| 20 | `kafka-listener-partitions-groupid-concurrency` | A | medium | high | `messaging-events/kafka-patterns` |
| 21 | `kafka-producer-batching-compression` | A | medium | high | `messaging-events/kafka-patterns` |
| 22 | `kafka-template-producer-factory-config` | A | medium | high | `messaging-events/kafka-patterns` |
| 23 | `retryable-topic-non-blocking-retry` | A | medium | high | `messaging-events/kafka-patterns` |
| 24 | `transactional-kafka-producers-spring` | A | medium | high | `messaging-events/kafka-patterns` |
| 25 | `dead-letter-exchange-retry` | A | medium | high | `messaging-events/message-guarantees` |
...
```jsonc
{
  ...
  "pillar": "P04",  // exactly this pillar code
...
_Topics in scope (codex `topic_id`s touched by this pillar's queue): 5._
...
[
  {
    "topic_id": "api-versioning",
    "example": "GET /v1/users vs GET /users with Accept: application/vnd.api+json;v=1",
    "use_when": "showing both schools side by side",
    "source": "agent-seeded"
  },
  {
    "topic_id": "circuit-breaker",
    "example": "Resilience4j CircuitBreaker around a downstream call to billing",
    "use_when": "the canonical resilience-pattern wrapping",
    "source": "agent-seeded"
  },
  {
    "topic_id": "cqrs-basics",
    "example": "OrderCommand (write) → OrderEventStore; OrderProjection (read) → denormalised view",
    "use_when": "the standard split shape",
    "source": "agent-seeded"
  },
  {
    "topic_id": "load-balancing",
    "example": "ALB at L7 routing /api/* to one target group, /static/* to another",
    "use_when": "the canonical L7 routing example",
    "source": "agent-seeded"
  },
  {
    "topic_id": "distributed-tracing",
    "example": "client → api → orders → payments — 4 spans in one trace",
    "use_when": "the canonical trace shape across hops",
    "source": "agent-seeded"
  }
]
```
...
- **Never** cross pillar boundaries. This brief is scoped to P04 only.
...
All 113 questions in scope, sorted by importance, then difficulty, then slug. Phase 3b iterates this list; Phase 3a smoke targets row 1.

| # | slug | archetype | confidence | importance | difficulty | module | topic |
|---:|---|---|---|---|---|---|---|
| 1 | `message-broker-vs-message-bus-vs-event-bus-semantics` | B | high | high | medium | `messaging-events` | `comparisons` |
| 2 | `kafka-log-compaction-and-retention` | B | medium | high | medium | `messaging-events` | `kafka-architecture` |
| 3 | `kafka-streams-kstream-ktable-fundamentals` | B | medium | high | medium | `messaging-events` | `stream-processing` |
| 4 | `webhook-reliability-retry-exponential-backoff` | A | medium | high | medium | `messaging-events` | `webhooks` |
| 5 | `webhook-security-hmac-signature-verification` | A | medium | high | medium | `messaging-events` | `webhooks` |
| 6 | `microservices-vs-serverless-vs-monolith-decision` | B | high | high | medium | `microservices` | `comparisons` |
| 7 | `orchestration-vs-choreography-saga-comparison` | B | high | high | medium | `microservices` | `comparisons` |
| 8 | `cqrs-with-event-sourcing-spring-boot` | A | medium | high | hard | `messaging-events` | `cqrs` |
| 9 | `kafka-broker-partition-leadership-isr` | A | medium | high | hard | `messaging-events` | `kafka-architecture` |
| 10 | `kafka-controller-election-kraft-vs-zookeeper` | B | medium | high | hard | `messaging-events` | `kafka-architecture` |
| 11 | `kafka-replication-and-durability` | A | medium | high | hard | `messaging-events` | `kafka-architecture` |
| 12 | `designing-idempotent-payment-microservice` | D | high | high | hard | `microservices` | `scenario-based` |
| 13 | `migrating-monolith-to-microservices-sequencing` | D | high | high | hard | `microservices` | `scenario-based` |
| 14 | `embedded-kafka-testing-spring-boot` | A | medium | high | intermediate | `messaging-events` | `kafka-architecture` |
| 15 | `acknowledgment-mode-manual-vs-auto` | B | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 16 | `kafka-consumer-group-rebalancing` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 17 | `kafka-consumer-lag-monitoring` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 18 | `kafka-headers-message-metadata` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 19 | `kafka-json-avro-serialization-spring` | B | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 20 | `kafka-listener-partitions-groupid-concurrency` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 21 | `kafka-producer-batching-compression` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 22 | `kafka-template-producer-factory-config` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 23 | `retryable-topic-non-blocking-retry` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 24 | `transactional-kafka-producers-spring` | A | medium | high | intermediate | `messaging-events` | `kafka-patterns` |
| 25 | `dead-letter-exchange-retry` | A | medium | high | intermediate | `messaging-events` | `message-guarantees` |
| 26 | `competing-consumers-pattern` | D | high | high | intermediate | `messaging-events` | `scenario-based` |
| 27 | `default-error-handler-seek-to-current` | D | high | high | intermediate | `messaging-events` | `scenario-based` |
| 28 | `api-gateway-spring-cloud-gateway` | A | medium | high | intermediate | `microservices` | `api-gateway` |
| 29 | `circuit-breaker-resilience4j` | A | medium | high | intermediate | `microservices` | `circuit-breaker` |
| 30 | `rest-vs-grpc-vs-messaging-interservice` | B | medium | high | intermediate | `microservices` | `communication-patterns` |
| 31 | `distributed-tracing-correlation-id` | A | medium | high | intermediate | `microservices` | `distributed-tracing` |
| 32 | `microservices-vs-monolith-when-to-split` | B | medium | high | intermediate | `microservices` | `fundamentals` |
| 33 | `strangler-fig-pattern-monolith-migration` | A | medium | high | intermediate | `microservices` | `migration-strategies` |
| 34 | `saga-pattern-distributed-transactions` | A | medium | high | intermediate | `microservices` | `saga-pattern` |
| 35 | `database-per-microservice-patterns` | A | medium | high | intermediate | `microservices` | `service-design` |
| 36 | `sidecar-pattern-service-mesh` | A | medium | high | intermediate | `microservices` | `service-mesh` |
| 37 | `dto-pattern-request-response` | A | medium | high | intermediate | `rest-api` | `api-design` |
| 38 | `rest-error-envelope-design` | A | medium | high | intermediate | `rest-api` | `api-design` |
| 39 | `rest-vs-websocket-vs-sse-realtime-patterns` | B | high | high | intermediate | `rest-api` | `comparisons` |
| 40 | `exception-handler-controller-advice` | A | medium | high | intermediate | `rest-api` | `error-handling` |
| 41 | `rfc7807-problemdetail-spring-boot` | A | medium | high | intermediate | `rest-api` | `error-handling` |
| 42 | `validation-error-response-shape` | A | medium | high | intermediate | `rest-api` | `error-handling` |
| 43 | `hateoas-spring-boot` | A | medium | high | intermediate | `rest-api` | `hateoas` |
| 44 | `openapi-swagger-springdoc` | A | medium | high | intermediate | `rest-api` | `openapi-and-swagger` |
| 45 | `file-upload-download-spring-boot` | A | medium | high | intermediate | `rest-api` | `request-and-response-handling` |
| 46 | `pagination-cursor-vs-offset` | B | medium | high | intermediate | `rest-api` | `request-and-response-handling` |
| 47 | `validation-valid-constraintvalidator` | A | medium | high | intermediate | `rest-api` | `request-and-response-handling` |
| 48 | `openfeign-integration-spring-cloud` | A | medium | high | intermediate | `rest-api` | `rest-client-and-feign` |
| 49 | `webclient-reactive-production-tuning` | A | medium | high | advanced | `rest-api` | `rest-client-and-feign` |
| 50 | `http-methods-get-post-put-delete-patch` | A | medium | high | intermediate | `rest-api` | `rest-fundamentals` |
| 51 | `http-status-codes-decision-guide` | A | medium | high | intermediate | `rest-api` | `rest-fundamentals` |
| 52 | `idempotency-in-rest-apis` | A | medium | high | intermediate | `rest-api` | `rest-fundamentals` |
| 53 | `put-vs-patch-http-difference` | E | high | high | intermediate | `rest-api` | `rest-fundamentals` |
| 54 | `what-makes-api-restful-constraints` | A | medium | high | intermediate | `rest-api` | `rest-fundamentals` |
| 55 | `protocol-buffers-define-generate-java` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 56 | `rest-api-security-cors-csrf` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 57 | `rest-rate-limiting-implementation` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 58 | `scaling-websockets-redis-pubsub` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 59 | `sockjs-fallback-older-browsers` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 60 | `spring-boot-websocket-stomp-setup` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 61 | `spring-message-broker-configuration` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 62 | `testing-websocket-endpoints-spring` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 63 | `websocket-handshake-upgrade-process` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 64 | `websocket-heartbeats-connection-management` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 65 | `websocket-security-authentication` | D | high | high | intermediate | `rest-api` | `scenario-based` |
| 66 | `websocket-vs-http-polling-vs-sse` | E | high | high | intermediate | `rest-api` | `scenario-based` |
| 67 | `websocket-vs-sse-detailed-comparison` | B | medium | high | intermediate | `rest-api` | `scenario-based` |
| 68 | `async-controllers-deferred-result-completable-future` | A | medium | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 69 | `content-negotiation-spring-mvc` | A | medium | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 70 | `cors-configuration-spring-mvc` | A | medium | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 71 | `handlerinterceptor-vs-filter` | B | medium | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 72 | `response-entity-spring-mvc` | A | medium | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 73 | `restcontroller-vs-controller-responsebody` | B | medium | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 74 | `spring-mvc-request-lifecycle` | C | high | high | intermediate | `rest-api` | `spring-mvc-controllers` |
| 75 | `api-versioning-uri-header-media-type` | B | medium | high | intermediate | `rest-api` | `versioning` |
| 76 | `spring-cloud-config-profiles-environment-specific-configuration` | A | medium | medium | easy | `microservices` | `config-management` |
| 77 | `feign-client-vs-resttemplate-vs-webclient` | B | medium | medium | easy | `microservices` | `feign-client` |
| 78 | `spring-cloud-loadbalancer-client-side` | A | medium | medium | easy | `microservices` | `load-balancing` |
| 79 | `kafka-vs-rabbitmq-vs-sqs-vs-pulsar` | B | high | medium | medium | `messaging-events` | `comparisons` |
| 80 | `event-schema-evolution-backward-compatibility` | A | medium | medium | medium | `messaging-events` | `kafka-architecture` |
| 81 | `consumer-group-fan-out-event-patterns` | A | medium | medium | medium | `messaging-events` | `kafka-patterns` |
| 82 | `exactly-once-vs-at-least-once-delivery` | B | medium | medium | medium | `messaging-events` | `message-guarantees` |
| 83 | `idempotent-consumers-event-deduplication` | A | medium | medium | medium | `messaging-events` | `message-guarantees` |
| 84 | `domain-events-vs-integration-events` | E | high | medium | medium | `messaging-events` | `messaging-fundamentals` |
| 85 | `event-driven-vs-request-driven-architecture` | B | medium | medium | medium | `messaging-events` | `messaging-fundamentals` |
| 86 | `spring-cloud-stream-event-driven-microservices` | A | medium | medium | medium | `messaging-events` | `spring-kafka` |
| 87 | `webhooks-design-reliability-security` | A | medium | medium | medium | `messaging-events` | `webhooks` |
| 88 | `spring-cloud-gateway-routing-filters` | A | medium | medium | medium | `microservices` | `api-gateway` |
| 89 | `resilience4j-circuit-breaker-spring-cloud` | A | medium | medium | medium | `microservices` | `circuit-breaker` |
| 90 | `spring-cloud-stream-messaging-abstraction` | A | medium | medium | medium | `microservices` | `communication-patterns` |
| 91 | `sync-vs-async-rest-grpc-messaging` | B | high | medium | medium | `microservices` | `comparisons` |
| 92 | `spring-cloud-config-server-centralized-configuration` | A | medium | medium | medium | `microservices` | `config-management` |
| 93 | `consumer-driven-contract-testing-pact-spring-cloud-contract` | A | medium | medium | medium | `microservices` | `contract-testing` |
| 94 | `pact-vs-spring-cloud-contract-comparison` | B | medium | medium | medium | `microservices` | `contract-testing` |
| 95 | `spring-cloud-contract-producer-consumer-testing` | A | medium | medium | medium | `microservices` | `contract-testing` |
| 96 | `distributed-tracing-micrometer-zipkin` | A | medium | medium | medium | `microservices` | `distributed-tracing` |
| 97 | `rate-limiter-resilience4j-spring-boot` | A | medium | medium | medium | `microservices` | `resilience-patterns` |
| 98 | `resilience4j-circuit-breaker-retry-bulkhead-timelimiter` | A | medium | medium | medium | `microservices` | `resilience-patterns` |
| 99 | `eureka-service-discovery-vs-consul` | B | medium | medium | medium | `microservices` | `service-discovery` |
| 100 | `rest-vs-graphql-vs-grpc-api-design` | B | high | medium | medium | `rest-api` | `comparisons` |
| 101 | `http-caching-etag-cache-control-spring-boot` | A | medium | medium | medium | `rest-api` | `http-caching-and-compression` |
| 102 | `resttemplate-webclient-feign-http-interface-comparison` | B | medium | medium | medium | `rest-api` | `rest-client-and-feign` |
| 103 | `cqrs-pattern-when-to-use-with-event-sourcing` | A | medium | medium | hard | `messaging-events` | `cqrs` |
| 104 | `event-sourcing-storing-state-as-events` | A | medium | medium | hard | `messaging-events` | `event-sourcing` |
| 105 | `outbox-pattern-reliable-event-publishing` | A | medium | medium | hard | `messaging-events` | `kafka-patterns` |
| 106 | `eventual-consistency-race-conditions-event-driven` | D | high | medium | hard | `messaging-events` | `scenario-based` |
| 107 | `saga-pattern-distributed-transactions-choreography` | D | high | medium | hard | `messaging-events` | `scenario-based` |
| 108 | `kafka-streams-vs-flink-vs-spark-streaming` | B | medium | medium | hard | `messaging-events` | `stream-processing` |
| 109 | `combining-resilience-patterns-order-of-operations` | A | medium | medium | hard | `microservices` | `resilience-patterns` |
| 110 | `idempotency-exactly-once-distributed-systems` | A | medium | medium | hard | `microservices` | `resilience-patterns` |
| 111 | `cascading-failure-one-slow-service-down-platform` | G | medium | medium | hard | `microservices` | `scenario-based` |
| 112 | `rest-resource-naming-conventions` | A | medium | medium | intermediate | `rest-api` | `api-design` |
| 113 | `http-compression-gzip-brotli-spring-boot` | A | medium | medium | intermediate | `rest-api` | `http-caching-and-compression` |

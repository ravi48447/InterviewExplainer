# Pillar P09 — DevOps agent brief

> Phase 3a/3b agent brief generated by `scripts/build_agent_briefs.py`. Self-contained: an agent reading only this brief, the lint script `scripts/audit_speakable.py`, the codex JSONs, and the 7 golden references embedded below can refine this pillar's queue to lint-green output. Phase 3a smoke gate: draft **only the first question** in `content/_audits/agent-queues/P09-queue.csv`, halt for human review. (Per the SPEAKABLE-PHASE-3A-PROMPT.md §1.5 + §7.4.)
...
- **Number of questions in scope** (excludes 7 goldens + 30 priority_handcraft + already-approved): **103**
- **Question list:** see `content/_audits/agent-queues/P09-queue.csv` (columns: slug, archetype, confidence, importance, difficulty, module, topic, file).
...
|---:|---|---|---|---|---|
| 1 | `docker-vs-vm-virtualization-comparison` | E | high | — | `docker/comparisons` |
| 2 | `docker-compose-spring-boot-postgres-redis` | A | medium | — | `docker/docker-compose` |
| 3 | `docker-health-checks-spring-boot` | A | medium | — | `docker/docker-fundamentals` |
| 4 | `docker-volumes-named-vs-bind-mounts` | B | medium | — | `docker/docker-fundamentals` |
| 5 | `docker-container-cant-reach-host-service` | D | high | — | `docker/scenario-based` |
| 6 | `jenkinsfile-structure-declarative` | A | medium | — | `jenkins/jenkins-pipelines` |
| 7 | `kubernetes-core-objects-pod-deployment-service` | A | medium | — | `kubernetes/kubernetes-workloads` |
| 8 | `github-actions-vs-jenkins-vs-gitlab-ci` | B | high | — | `cicd/comparisons` |
| 9 | `trunk-based-vs-gitflow-vs-github-flow` | B | high | — | `cicd/comparisons` |
| 10 | `blue-green-canary-rolling-comparison` | E | high | — | `cicd/deployment-strategies` |
| 11 | `feature-flags-trunk-based-development` | A | medium | — | `cicd/deployment-strategies` |
| 12 | `shift-left-testing-pipeline` | A | medium | — | `cicd/deployment-strategies` |
| 13 | `kubernetes-rollback-strategies` | A | medium | — | `cicd/rollback-strategies` |
| 14 | `cicd-pipeline-suddenly-slow-debug` | D | high | — | `cicd/scenario-based` |
| 15 | `docker-build-vs-buildah-vs-kaniko` | B | high | — | `docker/comparisons` |
| 16 | `docker-vs-podman-vs-containerd` | B | high | — | `docker/comparisons` |
| 17 | `debugging-spring-boot-in-docker` | D | medium | — | `docker/docker-fundamentals` |
| 18 | `docker-container-resource-limits-jvm` | A | medium | — | `docker/docker-fundamentals` |
| 19 | `jvm-memory-docker-container-limits` | B | medium | — | `docker/docker-fundamentals` |
| 20 | `docker-networking-bridge-host-overlay` | B | medium | — | `docker/docker-networking` |
| 21 | `docker-environment-variables-vs-secrets` | B | medium | — | `docker/docker-security` |
| 22 | `docker-image-scanning-security-best-practices` | A | medium | — | `docker/docker-security` |
| 23 | `docker-non-root-user-java-security` | A | medium | — | `docker/docker-security` |
| 24 | `docker-layer-caching-optimization` | A | medium | — | `docker/multi-stage-builds` |
| 25 | `multistage-dockerfile-spring-boot` | A | medium | — | `docker/multi-stage-builds` |
...
```jsonc
{
  ...
  "pillar": "P09",  // exactly this pillar code
...
_Topics in scope (codex `topic_id`s touched by this pillar's queue): 1._
...
[
  {
    "topic_id": "docker-layers-multi-stage-builds",
    "example": "FROM maven AS build → RUN mvn package → FROM eclipse-temurin:21-jre → COPY --from=build",
    "use_when": "the canonical Java multi-stage shape",
    "source": "agent-seeded"
  }
]
```
...
- **Never** cross pillar boundaries. This brief is scoped to P09 only.
...
All 103 questions in scope, sorted by importance, then difficulty, then slug. Phase 3b iterates this list; Phase 3a smoke targets row 1.

| # | slug | archetype | confidence | importance | difficulty | module | topic |
|---:|---|---|---|---|---|---|---|
| 1 | `docker-vs-vm-virtualization-comparison` | E | high | — | easy | `docker` | `comparisons` |
| 2 | `docker-compose-spring-boot-postgres-redis` | A | medium | — | easy | `docker` | `docker-compose` |
| 3 | `docker-health-checks-spring-boot` | A | medium | — | easy | `docker` | `docker-fundamentals` |
| 4 | `docker-volumes-named-vs-bind-mounts` | B | medium | — | easy | `docker` | `docker-fundamentals` |
| 5 | `docker-container-cant-reach-host-service` | D | high | — | easy | `docker` | `scenario-based` |
| 6 | `jenkinsfile-structure-declarative` | A | medium | — | easy | `jenkins` | `jenkins-pipelines` |
| 7 | `kubernetes-core-objects-pod-deployment-service` | A | medium | — | easy | `kubernetes` | `kubernetes-workloads` |
| 8 | `github-actions-vs-jenkins-vs-gitlab-ci` | B | high | — | medium | `cicd` | `comparisons` |
| 9 | `trunk-based-vs-gitflow-vs-github-flow` | B | high | — | medium | `cicd` | `comparisons` |
| 10 | `blue-green-canary-rolling-comparison` | E | high | — | medium | `cicd` | `deployment-strategies` |
| 11 | `feature-flags-trunk-based-development` | A | medium | — | medium | `cicd` | `deployment-strategies` |
| 12 | `shift-left-testing-pipeline` | A | medium | — | medium | `cicd` | `deployment-strategies` |
| 13 | `kubernetes-rollback-strategies` | A | medium | — | medium | `cicd` | `rollback-strategies` |
| 14 | `cicd-pipeline-suddenly-slow-debug` | D | high | — | medium | `cicd` | `scenario-based` |
| 15 | `docker-build-vs-buildah-vs-kaniko` | B | high | — | medium | `docker` | `comparisons` |
| 16 | `docker-vs-podman-vs-containerd` | B | high | — | medium | `docker` | `comparisons` |
| 17 | `debugging-spring-boot-in-docker` | D | medium | — | medium | `docker` | `docker-fundamentals` |
| 18 | `docker-container-resource-limits-jvm` | A | medium | — | medium | `docker` | `docker-fundamentals` |
| 19 | `jvm-memory-docker-container-limits` | B | medium | — | medium | `docker` | `docker-fundamentals` |
| 20 | `docker-networking-bridge-host-overlay` | B | medium | — | medium | `docker` | `docker-networking` |
| 21 | `docker-environment-variables-vs-secrets` | B | medium | — | medium | `docker` | `docker-security` |
| 22 | `docker-image-scanning-security-best-practices` | A | medium | — | medium | `docker` | `docker-security` |
| 23 | `docker-non-root-user-java-security` | A | medium | — | medium | `docker` | `docker-security` |
| 24 | `docker-layer-caching-optimization` | A | medium | — | medium | `docker` | `multi-stage-builds` |
| 25 | `multistage-dockerfile-spring-boot` | A | medium | — | medium | `docker` | `multi-stage-builds` |
| 26 | `docker-image-pull-rate-limit-fix` | D | high | — | medium | `docker` | `scenario-based` |
| 27 | `spring-boot-container-oomkilled-debug` | D | high | — | medium | `docker` | `scenario-based` |
| 28 | `git-merge-vs-rebase-vs-squash` | B | high | — | medium | `git-build-tools` | `comparisons` |
| 29 | `accidentally-committed-secret-to-git` | D | high | — | medium | `git-build-tools` | `scenario-based` |
| 30 | `maven-vs-gradle-vs-bazel` | B | high | — | medium | `java-build-tools` | `comparisons` |
| 31 | `maven-build-fails-only-on-ci` | D | high | — | medium | `java-build-tools` | `scenario-based` |
| 32 | `declarative-jenkins-pipeline-spring-boot` | A | medium | — | medium | `jenkins` | `jenkins-pipelines` |
| 33 | `jenkins-credentials-secrets-management` | A | medium | — | medium | `jenkins` | `jenkins-pipelines` |
| 34 | `jenkins-sonarqube-integration` | A | medium | — | medium | `jenkins` | `jenkins-pipelines` |
| 35 | `deployment-vs-statefulset-vs-daemonset` | B | high | — | medium | `kubernetes` | `comparisons` |
| 36 | `kubernetes-vs-docker-swarm-vs-nomad` | B | high | — | medium | `kubernetes` | `comparisons` |
| 37 | `gitops-argocd-flux-deployment-model` | A | medium | — | medium | `kubernetes` | `helm` |
| 38 | `helm-chart-structure-values-templating` | A | medium | — | medium | `kubernetes` | `helm` |
| 39 | `helm-release-lifecycle-upgrade-rollback` | C | high | — | medium | `kubernetes` | `helm` |
| 40 | `helm-vs-kustomize-when-to-use` | E | high | — | medium | `kubernetes` | `helm` |
| 41 | `kubernetes-configmaps-secrets-spring-boot` | A | medium | — | medium | `kubernetes` | `kubernetes-configuration` |
| 42 | `kubernetes-namespaces-rbac-multi-team` | A | medium | — | medium | `kubernetes` | `kubernetes-configuration` |
| 43 | `deploying-spring-boot-kubernetes-step-by-step` | A | medium | — | medium | `kubernetes` | `kubernetes-fundamentals` |
| 44 | `kubernetes-pod-networking-cross-node` | A | medium | — | medium | `kubernetes` | `kubernetes-networking` |
| 45 | `kubernetes-service-types-clusterip-nodeport-loadbalancer-ingress` | B | medium | — | medium | `kubernetes` | `kubernetes-networking` |
| 46 | `cluster-autoscaler-vs-karpenter-node-scaling` | B | medium | — | medium | `kubernetes` | `kubernetes-scaling` |
| 47 | `horizontal-pod-autoscaler-spring-boot` | A | medium | — | medium | `kubernetes` | `kubernetes-scaling` |
| 48 | `keda-event-driven-autoscaling-kafka-sqs` | A | medium | — | medium | `kubernetes` | `kubernetes-scaling` |
| 49 | `vertical-pod-autoscaler-vpa-sizing` | A | medium | — | medium | `kubernetes` | `kubernetes-scaling` |
| 50 | `kubernetes-persistent-volumes-statefulsets` | A | medium | — | medium | `kubernetes` | `kubernetes-workloads` |
| 51 | `kubernetes-resource-requests-limits-jvm` | A | medium | — | medium | `kubernetes` | `kubernetes-workloads` |
| 52 | `kubernetes-rolling-updates-rollbacks` | A | medium | — | medium | `kubernetes` | `kubernetes-workloads` |
| 53 | `liveness-readiness-startup-probes-spring-boot` | B | medium | — | medium | `kubernetes` | `kubernetes-workloads` |
| 54 | `crashloopbackoff-spring-boot-debugging` | D | high | — | medium | `kubernetes` | `scenario-based` |
| 55 | `pod-stuck-in-pending-debugging` | D | high | — | medium | `kubernetes` | `scenario-based` |
| 56 | `automated-rollback-on-slo-regression` | A | medium | — | hard | `cicd` | `rollback-strategies` |
| 57 | `cicd-secrets-leaked-in-build-logs` | D | high | — | hard | `cicd` | `scenario-based` |
| 58 | `jenkins-deploy-kubernetes` | A | medium | — | hard | `jenkins` | `jenkins-pipelines` |
| 59 | `spring-boot-oomkilled-kubernetes-debugging` | A | medium | — | hard | `kubernetes` | `kubernetes-fundamentals` |
| 60 | `kubernetes-service-mesh-istio-linkerd-when-needed` | A | medium | — | hard | `kubernetes` | `kubernetes-networking` |
| 61 | `dns-resolution-failing-kubernetes-coredns-debug` | D | high | — | hard | `kubernetes` | `scenario-based` |
| 62 | `matrix-strategy-multi-version-java-testing` | A | medium | — | intermediate | `cicd` | `cicd-fundamentals` |
| 63 | `trigger-workflows-pull-requests-protect-branches` | A | medium | — | intermediate | `cicd` | `cicd-fundamentals` |
| 64 | `cache-maven-gradle-dependencies-github-actions` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 65 | `deploy-spring-boot-github-actions` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 66 | `environment-specific-deployments-github-actions` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 67 | `github-actions-reusable-workflows` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 68 | `java-cicd-pipeline-github-actions` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 69 | `manage-secrets-github-actions` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 70 | `sonarqube-code-quality-github-actions` | A | medium | — | intermediate | `cicd` | `github-actions` |
| 71 | `git-hooks-code-quality` | A | medium | — | intermediate | `git-build-tools` | `code-quality-gates` |
| 72 | `git-bisect-find-bug-commit` | C | high | — | intermediate | `git-build-tools` | `git-internals` |
| 73 | `git-interactive-rebase-squashing-commits` | C | high | — | intermediate | `git-build-tools` | `git-internals` |
| 74 | `monorepo-vs-polyrepo` | B | medium | — | intermediate | `git-build-tools` | `git-internals` |
| 75 | `cherry-pick-vs-merge-vs-rebase-backporting` | B | medium | — | intermediate | `git-build-tools` | `git-workflows` |
| 76 | `git-flow-vs-trunk-based-development` | B | medium | — | intermediate | `git-build-tools` | `git-workflows` |
| 77 | `git-merge-vs-rebase` | E | high | — | intermediate | `git-build-tools` | `git-workflows` |
| 78 | `resolving-merge-conflicts` | A | medium | — | intermediate | `git-build-tools` | `git-workflows` |
| 79 | `gradle-build-script-spring-boot` | A | medium | — | intermediate | `java-build-tools` | `gradle-build` |
| 80 | `gradle-dependency-management` | A | medium | — | intermediate | `java-build-tools` | `gradle-build` |
| 81 | `gradle-incremental-build` | A | medium | — | intermediate | `java-build-tools` | `gradle-build` |
| 82 | `gradle-multi-module-project` | A | medium | — | intermediate | `java-build-tools` | `gradle-build` |
| 83 | `gradle-wrapper-why-use-it` | A | medium | — | intermediate | `java-build-tools` | `gradle-build` |
| 84 | `maven-dependency-scopes` | A | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 85 | `maven-lifecycle-phases` | C | high | — | intermediate | `java-build-tools` | `maven-build` |
| 86 | `maven-multi-module-project` | A | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 87 | `maven-pom-inheritance-aggregation` | A | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 88 | `maven-profiles-environment-builds` | A | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 89 | `maven-transitive-dependencies-conflict-resolution` | G | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 90 | `maven-vs-gradle-comparison` | B | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 91 | `publish-artifact-nexus-maven` | A | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 92 | `publish-java-library-maven-central-gradle` | A | medium | — | intermediate | `java-build-tools` | `maven-build` |
| 93 | `github-actions-vs-jenkins` | B | medium | — | intermediate | `jenkins` | `jenkins-pipelines` |
| 94 | `how-to-test-terraform-configurations` | A | low | — | intermediate | `terraform` | `terraform-core` |
| 95 | `provision-spring-boot-infrastructure-with-terraform` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 96 | `terraform-core-concepts-providers-resources-state-modules` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 97 | `terraform-data-sources-when-to-use` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 98 | `terraform-import-vs-terraform-taint` | B | medium | — | intermediate | `terraform` | `terraform-core` |
| 99 | `terraform-plan-and-apply-safely` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 100 | `terraform-state-locking` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 101 | `terraform-state-management-remote-state` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 102 | `terraform-variables-and-workspaces` | A | medium | — | intermediate | `terraform` | `terraform-core` |
| 103 | `what-is-terraform-module-how-to-create-one` | A | medium | — | intermediate | `terraform` | `terraform-core` |

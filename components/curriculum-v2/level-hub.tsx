            ...
            <header className="mb-6 rounded-xl border border-border bg-background/90 backdrop-blur-sm shadow-lg overflow-hidden">
              <div className="relative px-6 py-5 bg-surface border border-default">
                <div className="flex flex-wrap gap-2 mb-3">
                  <span className="text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg bg-blue-100 dark:bg-blue-950/20 text-primary dark:text-primary border border-default dark:border-default/20 shadow-sm">{curriculumToTitle(lang)}</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg bg-blue-100 dark:bg-blue-950/20 text-primary dark:text-primary border border-default dark:border-default/20 shadow-sm">{curriculumToTitle(track)}</span>
                ...
                <h1 className="text-3xl font-black tracking-tight text-foreground mb-3">{curriculumToTitle(lang)} {curriculumToTitle(track)}</h1>
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-800/60 bg-background px-3 sm:px-4 py-3">
                      <div className="flex flex-col gap-1.5">
                        {stack.questions.map((q, idx) => (
                          <Link key={`${idx}-${q.slug}`} href={`${basePath}/${stack.slug}/${q.slug}`} className="group/link flex flex-col sm:flex-row sm:items-center justify-between p-2.5 sm:pr-4 rounded-[8px] hover:bg-surface-subtle transition-all duration-200">

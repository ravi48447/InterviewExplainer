    ...
    <div className="min-h-screen bg-surface border border-default">
      ...
      <div className="w-full min-w-0 px-4 sm:px-6 lg:px-8 py-12">
        ...
        <header className="mb-10">
          <h1 className="text-4xl font-black tracking-tight text-foreground mb-3">{langTitle} {trackTitle} Interview Questions</h1>
        ...
        <section className="mb-12 grid grid-cols-1 sm:grid-cols-3 gap-6">
          {levels.map(({ key, meta, stacks }) => (
            <Link key={key} href={`/interview/${lang}/${track}/${key}`} className="group relative rounded-2xl border-2 bg-background p-6 hover:shadow-xl transition-all hover:scale-[1.02]" style={{ borderColor: meta.color + "80" }}>
              <div className="flex items-center justify-between mb-4">
                <span className={`px-3 py-1.5 rounded-full text-xs font-black border ${meta.colorClass}`}>{meta.label} · {meta.range}</span>
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {stacks.slice(0, 5).map((s) => (
                    <span key={s} className="text-[10px] px-2 py-0.5 bg-surface text-muted-foreground rounded-md font-medium">
                      {/* simple title-case inline */}
                      {s.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}
                    </span>
                  ))}
                  {stacks.length > 5 && (
                    <span className="text-[10px] px-2 py-0.5 bg-surface text-muted-foreground rounded-md">+{stacks.length - 5} more</span>
                  )}
                </div>
              )}
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Link href="/companies" className="flex items-center gap-4 p-5 bg-gradient-to-br from-orange-50 dark:from-orange-950/40  rounded-2xl border border-orange-200 dark:border-orange-500/20 hover:shadow-lg transition-all group  ">
            ...
            <div>
              <div className="font-black text-foreground group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">Company Prep</div>

...
export function buildIndex(contentDir) {
  ...
  // Canonical curriculum trees only (mirrors lib/content-reader.ts LOCKED_DOMAINS).
  // content/interview, content/domains and content/shared are LEGACY MIRRORS of
  // the curriculum trees (verified: interview/go/backend/beginner duplicates
  // go-fresher 146/146 topics) — indexing both would double-count every
  // question and collide ids.
  const CANONICAL_TREES = new Set([
    'frontend-fresher', 'frontend-intermediate',
    'go-fresher', 'go-intermediate',
    'java-backend-fresher', 'java-backend-intermediate',
    'java-fullstack-fresher', 'java-fullstack-intermediate',
    'python-backend-fresher', 'python-backend-intermediate',
    'ruby-backend-fresher', 'ruby-backend-intermediate',
  ]);
  ...
  const walk = (dir, depth, meta) => {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        const m = { ...meta };
        if (depth === 0) m.module = e.name;   // curriculum trees: content/<tree>/<module>/...
        else if (depth === 1 && !m.topic) m.topic = e.name; // topic dir under module
        walk(p, depth + 1, m);
        ...
        // Index EVERY question in the file, not just the first. The first
        // question keeps the legacy topic-level id (domain/module/topic) so
        // stored session records and concept topic lists stay valid; each
        // additional question gets a stable suffix id (-q2, -q3, ...).
        for (let qi = 0; qi < qs.length; qi++) {
          const q = qs[qi];
          const id = qi === 0 ? `${meta.domain}/${meta.module}/${topic}` : `${meta.domain}/${meta.module}/${topic}-q${qi + 1}`;
  ...
  // Walk each canonical curriculum tree with its domain pre-set
  // (content/<tree>/ is the DOMAIN; module dirs live one level below).
  for (const tree of CANONICAL_TREES) {
    walk(path.join(contentDir, tree), 0, { domain: tree });
  }

  // DSA problems have their own schema (problemStatement/approaches/patterns).
  // Index them as the 'dsa' domain so coding-mode sessions targeting DSA draw
  // REAL problems (statement + examples + starter code) instead of whichever
  // curriculum Q&A happens to contain code.
  const dsaDir = path.join(contentDir, 'dsa');
  if (fs.existsSync(dsaDir)) {
    const dsaCategories = fs
      .readdirSync(dsaDir, { withFileTypes: true })
      .filter((e) => e.isDirectory() && !e.name.startsWith('.') && !e.name.startsWith('_') && e.name !== 'basic-100' && e.name !== 'company' && e.name !== 'pattern' && e.name !== 'module' && e.name !== 'problem' && e.name !== 'sheet');
    for (const cat of dsaCategories) {
      const catDir = path.join(dsaDir, cat.name);
      for (const f of fs.readdirSync(catDir).filter((n) => n.endsWith('.json') && !n.startsWith('_'))) {
        let j;
        try {
          j = JSON.parse(fs.readFileSync(path.join(catDir, f), 'utf8'));
        } catch {
          continue;
        }
        if (!j?.problemStatement || !j?.approaches?.length) continue;
        const id = `dsa/${cat.name}/${j.slug ?? path.basename(f, '.json')}`;
        const approach = j.approaches[0];
        const codeByLang = approach.code ?? {};
        const codeExample = codeByLang.java ?? codeByLang.python ?? codeByLang.javascript ?? '';
        const spoken = [j.directAnswer, j.remember?.pattern, approach.insight].filter(Boolean).join('\n\n');
        const conceptsQ = (j.patterns ?? []).map((p) => ({ id: canon(p), label: p }));
        const tokens = tokenize([j.problemStatement, spoken].filter(Boolean).join(' '));
        corpusDocs.push(tokens);
        byDomain.dsa ??= {};
        byDomain.dsa[cat.name] ??= [];
        byDomain.dsa[cat.name].push(id);
        for (const c of conceptsQ) {
          const found = concepts.find((x) => x.id === c.id);
          if (found) found.topics.push(id);
          else concepts.push({ id: c.id, label: c.label, topics: [id], domains: new Set(['dsa']) });
          const ci = conceptIndex.get(c.id) ?? { topics: [] };
          ci.topics.push(id);
          conceptIndex.set(c.id, ci);
        }
        questions.push({
          ...
          domain: 'dsa',
          module: cat.name,
          topic: j.slug ?? path.basename(f, '.json'),
          question: String(j.problemStatement ?? ''),
          title: String(j.title ?? j.slug ?? 'Problem'),
          difficulty: String(j.difficulty ?? 'medium'),
          importance: j.frequency === 'very-high' || j.frequency === 'high' ? 'high' : 'medium',
          readingMinutes: Number(j.readingTimeMinutes ?? 8),
          concepts: conceptsQ.map((c) => c.id),
          conceptLabels: conceptsQ.map((c) => c.label),
          probes: [
            ...(j.followupVariations ?? []).slice(0, 3).map((t, i) => ({ kind: 'probe', id: `probe-${i}`, text: String(t) })),
            { kind: 'intent', id: 'intent-0', text: String(j.interviewerIntent?.testing ?? '') },
          ].filter((p) => p.text),
          mistakes: (j.commonMistakes ?? []).map(String),
          tradeoffs: [],
          tradeoffColumns: [],
          spoken,
          deep: approach.explanation ?? '',
          direct: String(j.directAnswer ?? ''),
          codeExample,
          codeExampleLines: codeExample ? codeExample.split('\n').filter((l) => l.trim() && !l.trim().startsWith('#') && !l.trim().startsWith('//')).length : 0,
          learnUrl: `/dsa/${cat.name}/${j.slug ?? path.basename(f, '.json')}`,
          isDsaProblem: true,
          examples: (j.examples ?? []).slice(0, 2).map((e) => ({ input: e.input, output: e.output })),

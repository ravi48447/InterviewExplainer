...
export class Director {
  constructor(opts) {
    ...
    const pool = this.interviewMode === 'behavioral'
      ? this.index.questions.filter(
          (q) =>
            /^(behavioral|fresher-behavioral-hr|behavioral-and-fresher-qa|soft-skills|hr)$/.test(q.module) ||
            /tell me about|describe a time|describe a situation|describe a project|walk me through (a|how you handled|your)|a conflict with|disagreement with|how you handled|your biggest|your greatest|a time you (failed|struggled|made a mistake)|your strengths|your weaknesses|proudest|most difficult decision/i.test(q.question)
        )
      : this.interviewMode === 'coding'
        ? (() => {
            // DSA sessions must ask REAL DSA problems (indexed with statements,
            // examples and starter code from content/dsa) — not whichever
            // curriculum Q&A happens to contain a code block. A coding mock on
            // a DSA domain (or an explicit 'dsa' domain) draws from the DSA pool;
            // curriculum coding sessions keep the code-example pool.
            const dsaPool = this.index.questions.filter((q) => q.isDsaProblem);
            const isDsaSession = opts.domain === 'dsa' || /(^|\/)dsa/.test(String(opts.domain ?? ''));
            if (isDsaSession && dsaPool.length) return dsaPool;
            const codePool = this.index.questions.filter((q) => (q.codeExampleLines ?? 0) >= 4);
            return codePool.length ? codePool : this.index.questions;
          })()
        : this.index.questions;
    this.queue = buildSessionQueue({
      index: { ...this.index, questions: pool },
    ...
    // coding mode: attach starter + language to each queued question
    if (this.interviewMode === 'coding') {
      this.queue = this.queue.map((q) => ({
        ...q,
        isCoding: true,
        starterCode: (q.codeExample ?? '')
          .split('\n')
          .map((l) => (l.trim().startsWith('#') ? l : ''))
          .filter(Boolean)
          .join('\n'),
        codingLang: /\bdef\b/.test(q.codeExample ?? '') ? 'ruby' : /\bfunc\b/.test(q.codeExample ?? '') ? 'go' : /class /.test(q.codeExample ?? '') ? 'java' : /def \w+\(|print\(/.test(q.codeExample ?? '') ? 'python' : 'javascript',
      }));
    }
    // behavioral mode: mark questions
    if (this.interviewMode === 'behavioral') {
      this.queue = this.queue.map((q) => ({ ...q, isBehavioral: true }));
    }
    this._first = true;
  }
  ...
  executeMove(move, ctx) {
    ...
    if (move === 'scaffold') {
      ...
      const easier = findScaffold(this.index, question, s.asked, this.rand);
      if (!easier) return this.executeMove('next', ctx);
    ...
    if (move === 'circle_back') {
      ...
      const orig = this.index.questions.find((q) => q.id === cb.questionId);
      const rephrase =
    ...
    if (move === 'pivot') {
      const piv = findPivot(this.index, question, s.asked, this.rand);
      if (!piv) return this.wrap('no pivot available');
      s.asked.add(piv.id);
